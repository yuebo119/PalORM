# PalORM 代码精炼系统（v7.0 — 引用 lessons.md 单一真源）

> `/refine` → **保持功能·更优实现**。不是修复缺陷，不是重构架构。
> **宗旨：质量为先**——扫描命中只是候选定位，执行前对候选点及其调用面**逐行阅读确认语义**；
> 精炼绝不以"扫描说可以改"为据直接改（O6/M9/A1 误报史证明扫描≠判断）。
> 核心：同样语义，更少代码·更高性能·更新特性。每项操作有量化效果预估。
> 基于精炼框架，适配 PalORM 插件-Provider 架构。
> **精炼约束**：参照 [`docs/编码规范.md`](../../docs/编码规范.md) — 违反规范的代码不精炼，先走 `/review`
...
| | `/review` | `/refine` | 边界 |
|------|:--:|:--:|------|
| null 检查缺失 | ✅ 发现 | ❌ | review 职责——找缺陷 |
| catch 无 OCE | ✅ 发现 | ❌ | review 职责——找缺陷 |
| `new List<T>()`→`[]` | ❌ | ✅ 精炼 | 精炼职责——现代化 |
| 集合无预分配 | ❌ | ✅ 精炼 | 精炼职责——提性能 |
| 架构违规 | ✅ 发现 | ❌ | 门禁职责 |
| 文件该不该删 | ✅ 判定（探针+误判库 P9 四类排除） | ✅ 执行删除 | 判定是 review 方法，执行归精炼——A1 十候选全证伪教训 |

**精炼不涉及的**：修复缺陷（/review）、架构决策评估（门禁已有）、跨层合并（破坏插件架构）。
...
基线刷新方式：`bash scripts/refine-scan.sh` 后更新本表。命中数明显变化（±30%）说明代码有大规模演化，先跑 /review 再精炼。
