# PalORM Review 系统（v3.0 — 统一入口）

> **审计系统与评审系统已彻底合并为本系统**（2026-07-19 裁决）。单入口 `/review`，按触发档位决定深度。
> 检查方法统一定义在 [`../deep-check-engine.md`](../deep-check-engine.md)（七流·probe-first·误判库·[推断]零容忍·危害×复杂度·下沉审查·四指标）。
> 本文件定义：触发档位、范围策略、产出格式、验证回路。
> 回答的问题：**diff 档 = 这次提交行不行？ 里程碑档 = 整体有什么缺陷、走向如何？**

---

## 触发档位（单入口 `/review`，按触及面自动升档，可显式指定）

| 档位 | 触发 | 范围 | 产出增量 |
|------|------|------|---------|
| **轻量** | diff 仅 docs/·*.md·注释 | doc-consistency-check.sh + 口径复核 | 不跑七流 |
| **轻量** | diff 仅 test/（不含测试基建） | assertion-strength-check.sh + 断言语义抽读 | 弱断言不得新增 |
| **标准** | diff 触及 src/（Core/Provider/SourceGen） | 七流 diff 范围 + 交叉验证；SourceGen 附专项路径（见下）；DataSession/SessionOperationState/Resilience 改动方法所在类全量 | 报告 8 段 |
| **全量** | 显式 `/review --full` 或重大重构后 | 七流全量逐行 + 覆盖度证据（零发现必须证明"深度检查后确认干净"） | 报告 8 段 + 覆盖度声明 |
| **里程碑** | 里程碑节点 `/review --milestone` | = 全量档 + 趋势 + 热点 | + metrics 四指标轮次对比行 + 热点表（见下） |
...
3. 范围声明三项（必须检查 / 明确不检查 / 抽样策略）——见引擎「执行前强制项」。
4. 防线代码 diff（快照/对称性/架构/性质测试、gate/assertion/doc 脚本）按 src 同深度逐行。
