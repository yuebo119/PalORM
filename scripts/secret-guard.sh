...
check_content() {
    ...
    # 38. 身份证号（18位）
    # 边界要求：身份证号是独立词法单位（前后非数字/小数点）。裸 [0-9]{17}[0-9Xx] 会把
    # 性能数据的长小数串误判为身份证——如 PerfHub 的纳秒值 "1119800123.456" 去掉小数点
    # 后连成 18 位连续数字（2026-09-21 实测误报，3 个 JSON 被拦）。
    # (?<![0-9.]) / (?![0-9.]) 排除「前后紧跟数字或小数点」的形态，真身份证号不受影响
    # （它不会嵌在更长数字串里）。
    echo "$content" | grep -inP '(?<![0-9.])[0-9]{17}[0-9Xx](?![0-9.])' > /dev/null 2>&1 && { echo "→ 身份证号"; return 1; }
    # 39. 银行卡号（16-19位连续数字）
