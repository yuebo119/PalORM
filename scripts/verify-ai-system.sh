...
for f in \
    ...
    .ai/review-system-v2.md \
    .ai/review-system-v2/template.md \
    .ai/review-system-v2/action-items-template.md; do
...
claimed=$(grep -hoE '[0-9]+ 条 STD 规则' \
    .ai/audit-system/prompt.md .ai/gate-system/prompt.md \
    .ai/refine-system/prompt.md .ai/review-system-v2.md 2>/dev/null \
    | grep -oE '[0-9]+' | sort -u)
...
pit_claimed=$(grep -hoE '[0-9]+ 项陷阱' .ai/audit-system/prompt.md .ai/gate-system/prompt.md .ai/review-system-v2.md 2>/dev/null | grep -oE '[0-9]+' | sort -u)
...
grep -q '113 API，112 实现' .ai/audit-system/prompt.md || api_ok=false
if $api_ok; then
...
kb_count=$(grep -cE '^### 模式 (P?[0-9]+)：' .ai/audit-system/known-false-positives.md)
