    ...
    where TProvider : IDbProvider
{
    ...
    //
    // 收益来自哪一半：三方交替 A/B（SQLite t2000）——
    //   只缓存参数、不缓存命令：TxHundredInserts 960 µs / 改前 969 µs（无收益），
    //   分配 223055 vs 254628 B（只回收 12%）
    //   命令 + 参数都缓存：TxHundredInserts 549 µs、分配 154666 B（回收 39%）
    // 即**命令的新建与释放才是大头**，只池化参数几乎白做。
    //
    // 为什么必须惰性晋升（第 3 次调用才缓存命令）：缓存命令意味着"命令只随会话释放"，
    // 而 DataSession 构造时接管所接收连接的所有权——"一条外部连接配多个一次性会话"的用法
    // （PerfHub 的每操作一会话正是如此）无法调用 DisposeAsync，那会把共享连接一起关掉。
    // 无条件缓存命令时该用法每次操作泄漏一个未释放的命令（原生语句句柄 + 终结器），
    // 实测并发混合负载慢 4.84×（40380 vs 8335 µs，三轮交替每臂 6 跑）。
    // 阈值 3 让"每操作一个抛弃式会话"永远不进缓存（既不泄漏也不退回原性能），
    // 而真正复用会话的用法（100/500 次连续写）从第 3 次起拿到全部收益。
    //
    // 为什么能复用：同 (实体类型, 操作) 的语句与参数名/序/类型逐位相同，变的只有值；
    // 会话写路径由 SessionOperationState 单活动操作门禁串行化，不存在并发复用同一命令。
    //
    // 为什么池由首次按原绑定路径摘出：名/类型/精度与逐行 BindInsert/BindUpdate 逐位
    ...
    /// <para>前 <see cref="CommandReusePromotionThreshold"/>-1 次返回 null，调用方走新建路径
        ...
        where T : class, new()
    {
        ...
        servedOps++;
        if (servedOps < CommandReusePromotionThreshold)
            return null;
    ...
    /// <para>晋升阈值与租户/旧模型程序集的排除见 <see cref="TryAcquireInsertCommand"/> 与本节注释。</para>
    /// <para>返回的命令归会话所有（会话释放时统一 Dispose），调用方不得释放。</para></summary>
    private DbCommand? TryAcquireUpdateCommand<T>(
        string updateSql, CrudMetadata metadata, T entity, ref int servedOps)
        where T : class, new()
    {
        ...
        List<Action>? deferredVersionIncrements = null) where T : class, new()
    {
        ...
        // PL-2：命令/参数复用路径。未晋升（前两次、租户实体或旧模型程序集）时返回 null，
        // 走下方新建路径——命令随操作释放，不泄漏。
        if (TryAcquireUpdateCommand(updateSql, metadata, entity, ref _updateWriteOps) is { } reusedUpdate)
        {
