...
internal static class SqlFileEmitter
{
    ...
    {
        // 使用 C#11 raw string literal (""") 处理含引号和反斜杠的 SQL
        ...
        sb.Append("    public static partial string ").Append(methodName).AppendLine("() => \"\"\"");
    ...
    private static string GenerateError(IMethodSymbol method, string error)
    {
        ...
        sb.AppendLine($"partial class {typeName}");
        sb.AppendLine("{");
        sb.AppendLine($"    /// <summary>SqlFile 源生成失败: {error}</summary>");
        sb.AppendLine($"    [global::System.Obsolete(\"SqlFile 源生成失败: {error}\", error: true)]");
        sb.Append("    public static partial string ").Append(method.Name).AppendLine("() => throw new global::System.IO.FileNotFoundException(\"").Append(error).AppendLine("\");");
        sb.AppendLine("}");
