...
- **上一轮新增的 `RegistryIncrementalCostTests` 是 flaky 的**（5 轮红 4）：它用同一进程内的
  ...
  **反而更糟**——失败输出给了真相：同一比值在 0.2~0.8 之间跳。
  ...
  完全不受负载影响。5 轮连绿。
- **订正 B4 解读（原解读错在把比值当成粒度证据）**：对象同一性实测给出确定的粒度事实——
  · 输入未变重跑：**全部 361 个输出文件复用同一批实例**（缓存生效的确定证据，10 ms）
  · 改一个实体：**361 个文件全部是新实例，一个都没复用**（154 ms / 冷跑 829 ms ≈ 1/5）
  <br>即生成器把所有逐实体产物与聚合注册表放在**同一个输出节点**上，任何实体变化都让该节点
  整体重跑并把结果重新物化——**增量粒度是全有或全无**，不是"逐实体产物按实体缓存"。
  代价仍远低于冷跑（上游模型构造/SQL 构建/语法解析确实被缓存，重跑的只是物化），
  但这意味着 500 实体工程的单次编辑仍要重物化约 1500 个源文件。
  <br>可能的改进方向（未实施，需另行评估）：把逐实体产物改为**每个实体一个 `RegisterSourceOutput`
  节点**，让 Roslyn 能按实体缓存——比"注册表按块拆多文件"更贴合真实代价分布。
  本用例的打印会直接反映该改动是否生效（复用数应从 0 上升）。
