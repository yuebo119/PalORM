...
- **B4（增量重生成）不再按原假设处理，实测修正了假设**：原判断是"注册表是聚合输出，
  ...
  | 改一个实体 | **131 ms** | 冷跑的 **20%** |
  <br>即：逐实体产物（RowFactory/CommandFactory/Migration）确实命中缓存，只有注册表这一份
  聚合产物重发，份额约 20%（不是 100%）。500 实体规模按此推算单次编辑约数百毫秒——可接受，
  故**不建议**为它拆成按块多文件输出（那会改变生成物文件集，牵动快照与文档；
  收益与风险不成比例）。守卫同时钉住"输入未变必须走缓存"与"改一个实体不得退化为冷跑量级"。
