...
## 九-B、复测登记（2026-09-26）：并发 1.25× 与强优三行的归因

> 触发：基线汇报标“未复测不归因”的两处读数。复测批1 = `ab/recheck-sqlite-1`（history-20260926-124922，同代码、全档并发，ab 标签不顶 latest）。

**① 并发 Mixed80_20 1.25×——稳定真差，非噪声**：两批 P/ADO 1.23~1.25（地板臂 6300→6100µs 仅 −3% 波动，量具复现性达标）。归因链（探针量化）：
- PerfHub PalORM 臂按“每操作一会话”形态跑（`Implementations.cs` Session()，与 Dapper 无状态扩展对等），地板臂为复用长连接——形态公允差，非产品缺陷；
- 探针：每操作一会话的 `CreateAsync` 比裸连接贵 **~16µs/op**（PRAGMA batch + 会话建立；PRAGMA 文本已 const 化后仍是 batch 成本）；
- 探针：默认 `MaxRetries=3` 的弹性机构 **2~29µs/op**（4 线程并发形态，两轮 A/B，首轮偏大含 JIT warmup，次轮归零）。
