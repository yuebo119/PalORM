...
| M2-2 | P2 | 批量 UPDATE 参数化分支真库端到端（PG+MySQL 各 ≥3，逐行逐列断言） | 测试已立，排除矩阵定格 | 稳定复现：多行（4 实体）PG 42601/MySQL 语法错；单行经 PalORM 真库成功。十一项排除矩阵全部完成：SQL 文本（1/4 行 dump 同构）、参数名数（@p0-@p11）、两种绑定方式、事务有无、auto-prepare 两项复刻、同语句两次执行、NpgsqlParameter(name,value) 构造器、CreateConnection 全量调优复刻（NoResetOnClose/16384 缓冲区/Enlist=false）、初始化钩子（PG 未覆写=no-op）、码点裁决（LEN=203 零非 ASCII）、参数值裁决（10,u1,1/… 行主序全对）——发送前状态与原生成功版完全等价，42601 在原生路径不可复现。分叉仅存于 Npgsql 内部会话状态或连接历史语句序列（原生对照连接只跑过 DDL，PalORM 连接有参数化语句历史）。下一招：cmd.Clone() 换新连接执行（分离命令对象来源 vs 连接历史）/ Npgsql 网络层日志 |
