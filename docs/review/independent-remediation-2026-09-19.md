...
| M2-2 | P2 | 批量 UPDATE 参数化分支真库端到端（PG+MySQL 各 ≥3，逐行逐列断言） | 测试已立，十五项矩阵定格 | 稳定复现：多行（4 实体）PG 42601/MySQL 语法错；单行成功。**十五项排除矩阵 + 管线三个内部方法全读（ExecuteBatchUpdateAsync/RunInTransactionScopeAsync/BeginTransactionCoreAsync）**：SQL 文本、参数名、参数值、两种绑定、事务有无、auto-prepare 复刻、同语句两次、构造器、全量调优、初始化钩子、码点、连接语句历史（DDL+4 次参数化 INSERT+SELECT）、conn.CreateCommand 来源、同名表（palorm_batch_upd）、终极组合（同名表+CreateCommand+显式事务）——原生全组合成功，与 PalORM 分叉无法在可见层定位。下一步（需网络层证据）：①Npgsql 日志看实际发送字节 ②cmd.Clone 跨连接交叉执行（需临时 src 挂钩） ③对比 PalORM 与原生的 Npgsql InternalCommand 行为差异 |
