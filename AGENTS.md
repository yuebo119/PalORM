...
| **高** | `.ai/lessons.md` | 规范系统手册 v7.14（93 缺陷：A1-A7 + B1-B86 + XI 性能测量纪律 SOP，本地工具，不入仓库） |
| **高** | `.ai/test/prompt.md` | 测试规范系统 v1.1（18 铁律 + 17 缺陷，本地工具，不入仓库） |
...
| `/test` | 测试规范 | T1-T18 测试铁律 + 覆盖矩阵 + 基准配置规范 |
...
4. **色标分档**（时延 P/ADO% 与分配相对 ADO% 共用；+ = 劣于 ADO 取暖色、− = 优与 ADO 取冷绿，各三级，+30% 对齐门禁 1.3× 线）：
