...
| MySQL BulkCopy（4 列，去 DataRow 层） | 294.1 B/行 | **105.9 B/行** | **−64%** |
| MySQL BulkCopy（19 列，去 DataRow 层） | 940.3 B/行 | **502.0 B/行** | **−47%** |
| 批量 UPDATE 单语句（PG，40000 行） | 2750.0 B/行 | **1609.2 B/行** | −41% |
| 批量 UPDATE 单语句（MySQL，40000 行） | 2457.0 B/行 | **2006.4 B/行** | −18% |

最后两行是紧接着的第二轮（v5.6 续）：参数池到位后，**DataRow 层成了唯一的每行分配主项**——
逐行 `table.NewRow()` + 逐列写值实测 372.0/360.0 B/行，改成自实现 `DbDataReader` 按序号直读
参数池后为 57.0 B/行（−84%），且**时间同向更快**（114.2/112.7 → 100.9/100.4 ms，−11%）。
原注释断言"MySqlBulkCopy 对 DataTable 路径有专门优化"——实测证伪。真库复核见上表。
...
- **仪器已验证**：阳性对照通过（27 项 `exit 0`）；四处变异全部被抓到——
  ...
  平台提示另做过两条探针：基线声明为 `Ubuntu 24.04.1 LTS` 而当前为 Windows → 打印
  「录制平台与当前不同（Linux / Windows）」警告行；基线为 Windows 而当前为 Windows → 无警告。
  三个阈值的实测噪声底与余量见下节。

### 阈值标定：三个阈值各自的实测噪声底（2026-09-16）

阈值不能凭感觉定，也不能凭"看起来合理"定。用**同一份代码连跑 3 轮**（每轮 27 项，
与门禁完全同参：`--launchCount 1 --warmupCount 3 --iterationCount 5`）测出每个维度的
噪声底，再让阈值站在噪声之上：

| 维度 | 阈值 | 同代码 4 轮间的实测波动 | 余量 |
|---|---|---|---|
| 分配字节数 | +20% | **逐位相同**；仅 `PalORM_QueryAll` 在 1547164 ± 30 B 浮动（±0.002%） | 极大（保留原因见下） |
| 分配比 | +10% | 3.2879 / 4.3353 / 1.1373 / 8.0951 / 2.9791 / 3.3828 —— 三轮**四位小数全同** | 极大 |
| 耗时比 | +30% | 相对基线的偏离 −9.4% ~ **+11.1%**（3 轮 × 6 项 = 18 个采样） | **2.7 倍** |

耗时比的逐项波动（三轮 min→max）：`StableShape` 1.208→1.220（1.0%）、`Update` 1.288→1.344
（4.4%）、`GetByKey` 1.132→1.223（8.0%）、`QueryAll` 1.052→1.138（8.2%）、
`Insert` 1.348→1.495（10.9%）、`VaryingShape` 1.245→1.438（15.5%）。
**同一份代码的耗时比本身就能动 15.5%**，所以任何小于 20% 的耗时比阈值都只是在测抖动；
+30% 是最坏偏离的 2.7 倍，它挡的是量级性 CPU 退化，不是 10% 级的抖动。

同代码重跑门禁自检：**27/27 PASS**（含上表里最坏的 `VaryingShape` 1.438/1.294 那一次）。
阳性对照：把基线里 `VaryingShape` 的耗时比压到 1.0 → **`exit 1`**，且只有该项失败。

分配维度实测近乎零噪声（±0.002%），+20% 的阈值因此显得过分宽松——**保留不收紧的原因是跨平台**：
本基线录于 Windows，而 workflow 跑在 `ubuntu-latest`，跨平台分配一致性尚未验证
（本机无 Docker，无法本地复现）。判定工具因此新增一条提示：基线录制平台与当前平台不同族时
先打印警告行（`Windows` / `Linux` / `macOS` 三族，按 `RuntimeInformation.OSDescription` 归一——
注意 Ubuntu 上该字段是 `"Ubuntu 24.04.1 LTS"`，字面不含 `Linux`，只认关键词会漏判，
且会把两个 Ubuntu 补丁版本判成不同族而误报）。**待办**：首次 CI 运行后核对分配字节数与
基线是否一致；一致即可把分配阈值收紧到 +5%，并以 CI 平台重录基线。
...
## 🌐 跨方言并发首数据（2026-09-19，`--workload pg/mysql`，维度 4）

S1 × 10,000 · 80/20 读写 · 每档预热 1.5 s + 测量 3 s · 远程真库（LAN）。
**单轮首数据，只登记结构结论**（绝对值受服务器状态影响；扩展比为同轮可比）。

| 线程 | SQLite（本机）ops/s | PG（远程）ops/s | MySQL（远程）ops/s |
|---:|---:|---:|---:|
| 1 | 88,552 | 1,717 | 1,633 |
| 2 | 75,227 | 3,955 | 2,329 |
| 4 | 66,477 | 7,012 | 4,409 |
| 8 | 34,617（回落） | **11,606（+6.8×）** | **7,316（+4.5×）** |

结构结论：**SQLite 的 8 线程回落是方言形状而非 ORM 瓶颈**——远程 PG/MySQL 随线程
近线性扩展（PG 4 线程 4.1×、8 线程 6.8×）。本机 SQLite 单线程快 50× 是无网络往返的
本地效应；跨方言对比须在同方言内看扩展比。PG 分位数 p50 0.35–0.58 ms / p99 1.1–3.9 ms，
MySQL p50 0.32–0.59 ms / p99 2.4–4.5 ms（远程 RTT 主导）。

## 📊 规范化全量测评（2026-09-16，规范首跑）

按 `docs/性能基准规范.md` 执行的全量 sweep。SQLite 本机档 + 远程真库档；
门禁判定 **27/27 阈值内**（分配与基线逐项 0% 漂移 → 无回归）。

**查询构建分配**（S1，20 万次/点）：`From<T>()` 64.0 B/次（逐位复现）；
`Where + ToSql` 合计 968 B/次（新口径——含 Where 子句与参数构造，历史 392 B 是不含 Where 的纯 ToSql，两者不可比）。

**大结果集内存曲线**（S1，全量物化）：

| 档位 | 分配总量 | 实体存活 | 耗时 |
|---:|---:|---:|---:|
| 10,000 | 1.4 MB | 1.0 MB | 54–64 ms |
| 100,000 | 13.4 MB | 9.3 MB | 224–530 ms |

存活内存 ≈ 0.093 KB/行，线性。**审计发现：PalORM 无真流式公共 API**——
`GridReader.ReadAsync<T>()` 亦为整集物化（GridReader.cs:44），
规范维度 7 的"流式对比"即因此不可做，记为已知形态而非缺陷。

**负载测试（本轮两轮 + 历史两轮，共 4 轮）**：稳态 27–30K ops/s @1–2 线程
（首轮 54,626 为构建后热态离群）；8 线程结构性回落至 11–19K（SQLite 单写者 + WAL）；
p50 0.02–0.03 ms · p95 0.05–0.06 ms · p99 0.07–0.11 ms（4 轮一致）。

**远程批量（真库档）**：PG 分配与基线**逐位吻合**（211.1 / 1512.9 / 1609.3 B/行 ✓）；
MySQL 批量插入 **70.1 B/行 vs 上轮 105.9（−34%，超规范 1% 红线，待查明）**；
宽实体 477.0 vs 502–527（LOAD DATA 网络缓冲假说，±5–9% 波动）。
**本轮全部时间数字不可比**：远程服务器深度降级（MySQL 多批 UPDATE 21.5 s vs 历史 0.4–6 s）。

## 🔀 并发负载测试（2026-09-16 首跑，`--workload`）

> 规范依据：`docs/性能基准规范.md`（维度 3 延迟分布 / 4 并发扩展 / 11 混合负载）。
> 形状 S1 Narrow × 10,000 行 · 读写比 80/20 · 每档预热 1.5 s + 测量 3 s · SQLite 文件库 WAL ·
> Windows 10 / .NET 11.0.0-rc.1（环境节详见规范 §3 要求）。
> **首跑数据，仅登记不设阈值**——分位数门禁待积累噪声底后再定（规范 §6 schema 2）。

**第 1 轮**（紧接构建，机器热态）：

| 线程 | ops/s | p50 (ms) | p95 (ms) | p99 (ms) | 样本数 |
|---:|---:|---:|---:|---:|---:|
| 1 | 54,626 | 0.01 | 0.03 | 0.07 | 81,937 |
| 2 | **71,711** | 0.01 | 0.02 | 0.02 | 110,945 |
| 4 | 55,860 | 0.01 | 0.02 | 0.02 | 86,548 |
| 8 | 25,492 | 0.01 | 0.02 | 0.02 | 39,404 |

**第 2 轮**（约 20 分钟后，同机同参）：

| 线程 | ops/s | p50 (ms) | p95 (ms) | p99 (ms) | 样本数 |
|---:|---:|---:|---:|---:|---:|
| 1 | 29,075 | 0.03 | 0.05 | 0.07 | 43,119 |
| 2 | 28,885 | 0.02 | 0.05 | 0.09 | 44,720 |
| 4 | 24,476 | 0.02 | 0.05 | 0.07 | 41,560 |
| 8 | 19,493 | 0.02 | 0.05 | 0.09 | 33,705 |

解读与教训：
- **运行间方差远大于轮内**：1 线程 ops/s 两轮相差 1.9 倍（54,626 vs 29,075），p50 也翻倍
  ——负载测试的运行间方差必须按**多轮中位数**标定，单轮 ops/s 不得作为结论
  （这正是规范 §4"统计纪律"的活例；第 1 轮恰是较快的一轮，若只发它就高估了近一倍）。
- 两轮一致的**结构性**结论：8 线程吞吐不升反降、p99 相对稳定——SQLite 单写者 + WAL
  锁竞争的形状，不代表 PG/MySQL；跨方言档位是 `--workload` 的下一步。
- 第 1 轮 p50 ≈ 0.01 ms 与读路由优化后的单查询 8.98 µs 独立互证（量级一致）。
