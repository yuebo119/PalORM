...
解读：吞吐峰值在 **2 线程**（+31% vs 单线程），8 线程回落到单线程的一半以下——
这是 SQLite 单写者 + WAL 锁竞争的形状（写占 20%，写锁串行化成为瓶颈），
不代表 PG/MySQL 下的扩展性；跨方言档位是 `--workload` 的下一步。
p50 ≈ 0.01 ms 与读路由优化后的单查询 8.98 µs 交叉吻合（两个独立量具互相印证）。
