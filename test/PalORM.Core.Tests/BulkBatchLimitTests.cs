...
/// （2026-09-25 起 32766——引擎编译选项 MAX_VARIABLE_NUMBER=32766 探针实测；原 999）后，
/// 多批删除与多批 upsert 的正确性——用例行数随之提高到仍跨多批（批大小=min(32766,5000)）。
/// 若参数上限越界，会在这些用例里先炸。</para>
...
internal sealed class BulkBatchLimitTests
{
    ...
    {
        // 11000 键：批大小 = min(32766, 5000) = 5000 → 3 批；批大小越界会在此失败
        // （999 时代的 2500 键在新上限下单批装下，多批正确性验证面随之提高行数）
        await using DataSession<SqliteProvider> session = await CreateSessionAsync();
        List<BulkLimitRow> entities = BuildRows(11000, "t");
        ...
        await Assert.That(deleted).IsEqualTo(11000);
    ...
    {
        // 11000 行 × 3 列 = 33000 参数 > 32766：批大小 = 32766/3 = 10922 → 2 批
        await using DataSession<SqliteProvider> session = await CreateSessionAsync();
        List<BulkLimitRow> entities = BuildRows(11000, "a");

        await session.BulkMergeAsync(entities);
        await Assert.That((await session.GetAllAsync<BulkLimitRow>()).Count).IsEqualTo(11000);
        ...
        await Assert.That(rows.Count).IsEqualTo(11000);
